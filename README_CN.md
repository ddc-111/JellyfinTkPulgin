...
- **多模态 AI 分析** — 集成小米多模态大模型，分析视频帧内容，自动生成标题、语义标签和情感标签
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| 启用多模态分析 | 关闭 | 开启 AI 语义分析功能 |
| API Base URL | `https://api.mimodel.com/v1` | 小米多模态大模型 API 地址 |
| API Key | (空) | 小米 API 密钥 |
| 模型名称 | `MiMo-VL-7B` | 多模态模型标识符 |
| 采样帧数 | 3 | 每个片段抽取的帧数（1-8） |
